'''
Simple phonebook used python-shelve.
'''
...
SayNumberInfo = '''Номер '%s' принадлежит контакту %s, проживающему
                по адресу: %s.\n'''
SayNameInfo = '''Контакту %s, проживающему по адресу: %s, принадлежит
                номер '%s'.\n'''
ErrorLocation = '''В телефонной книге обнаружено несколько
                пользователей с таким именем. Пожалуйста,
                укажите адрес:\n'''
...
menu = '''Добро пожаловать в телефонную книгу!\nГлавное меню\n
        ...
        чтобы удалить номер из телефонной книги\n'3' чтобы найти
        номер по имени в телефонной книге\n'4' чтобы просмотреть все
        записи\n'5' чтобы найти имя по номеру в телефонной книге\n'0'
        чтобы выйти из телефонной книги\n'''
db = shelve.open(filename)


def add(db):
    '''
    Add new user into the phonebook.
    '''
    name = input(AskName)
...
def delete(db):
    '''
    Delete user with the number from the phonebook.
    '''
...
def searchByPhone(db):
    '''
    Return `phone`, `name` and `location` of user with
    the number.
    '''
    phone = input(AskNumber)
    if (phone in db):
